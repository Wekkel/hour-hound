...
function openOverboekPost(targetId){
  const os=HH.state.read().overbookings.filter(o=>overboekingOpen(o)&&o.targetDossierId===targetId);
...
async function handelOverboekingenAf(){
  const ids=overboekPostIds.slice(),os=ids.map(id=>HH.state.read().overbookings.find(o=>o.id===id)).filter(Boolean);
...
async function verversOverboeking(id){
  const o=HH.state.read().overbookings.find(x=>x.id===id);if(!o||!overboekingOpen(o))return;
...
async function maakOverboekingDefinitiefI7(id){
  const o=HH.state.read().overbookings.find(x=>x.id===id);if(!o||!overboekingOpen(o))return;
  const rs=bronIdsVan(o).map(rid=>HH.state.read().rules.find(r=>r.id===rid)).filter(Boolean),ind=i7();
  ...
  if(rs.some(r=>!r.eind||(HH.state.read().running&&HH.state.read().running.id===r.id))){toast("Stop eerst alle betrokken timers");return;}
