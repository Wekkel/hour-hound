...
function harness({count=1,booked=false,alias=false,fail=false,parkFail=false}={}){
...
for(const alias of [false,true])await test('Numbered DVN parking copies actual Intapp fields'+(alias?' through alias':''),async()=>{const h=harness({alias});h.open();assert.equal(h.els.get('bk-park').style.display,'');h.els.get('bk-park').onclick();for(const id of ['pb-i7-copy','pb-code-copy','pb-hours-copy','pb-copy'])await h.els.get(id).onclick();assert.equal(h.copies[0],'I700000000');assert.equal(h.copies[1],'Commercieel');assert.equal(h.copies[2],'0,6');assert.ok(h.copies[3].startsWith('Tijdelijk i7 voor 304000001 · Bank - Project · '));assert.ok(h.copies[3].includes('Werk 0'));await h.els.get('pb-save').onclick();assert.equal(h.H.state.read().overbookings.length,1);assert.equal(h.els.get('bk-copy').disabled,true);});
await test('Parking cancelled when the same source was booked elsewhere',async()=>{
 const h=harness({parkFail:true});h.open();h.els.get('bk-park').onclick();
 await h.els.get('pb-save').onclick();
 assert.equal(h.H.state.read().overbookings.length,0);
 assert.equal(h.run('parkBoek'),null);
});
