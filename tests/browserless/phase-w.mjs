...
test('via Dag geboekte DVN verschijnt afgehandeld',async()=>{const dvn={...d,dvn:true,dvnResolvedNr:d.nummer,voorlopig:false},h=harness({dossiers:[dvn]}),real=loadRealAdapters(h),c=h.HH.__context;h.HH.state.commit({dossiers:[dvn]});const out=await h.HH.services.admin.setRegularBooking({...h.input(),...real,snapshot:real.snapshotRow(vm.runInContext('sumVanData(HH.state.read().rules)[0]',c),date,[rule],'groep')});assert(out.ok,'dagboeking');h.HH.state.commit({bookingHistory:out.history,booked:out.booked});equal(vm.runInContext('dvnIntappState(HH.state.read().dossiers[0])',c),'posted','DVN spreekt Dag tegen');});
