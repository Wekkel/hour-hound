...
function ioRuntime(h){loadRealAdapters(h);const c=h.HH.__context;c.confirm=()=>true;c.prompt=()=>null;c.zorgVoorI7=async()=>{};c.laadInstellingen=async()=>{};c.herlaad=async()=>{};c.Blob=Blob;
 ...
 return{c,check:vm.runInContext('keurBookingHistory',c),merge:vm.runInContext('voegBookingHistorySamen',c),checksum:vm.runInContext('checksumVan',c),importFile:vm.runInContext('importFile',c),export:async()=>{await c.document.getElementById('b-export').onclick();return JSON.parse(await blob.text());}};
}
...
test('echte export en restore bewaren schema10 en volledige historie',async()=>{const h=harness(),io=ioRuntime(h),history={...h.b.emptyHistory(),receipts:[record(h,h.snap())]};h.db.rows.meta.set('bookingHistory',history);const dump=await io.export();equal(dump.schemaVersion,10,'schema niet verhoogd');equal(JSON.stringify(dump.meta.bookingHistory),JSON.stringify(history),'export mist historie');
 const restored=harness({regels:[],dossiers:[]}),target=ioRuntime(restored);await target.importFile({size:100,text:async()=>JSON.stringify(dump)});equal(JSON.stringify(restored.db.rows.meta.get('bookingHistory')),JSON.stringify(history),'restore verloor historie');});
...
test('schema9 samenvoegen bewaart W-historie en terugzetten waarschuwt',async()=>{
 ...
 for(const restore of [false,true]){const h=harness(),target=ioRuntime(h),history={...h.b.emptyHistory(),receipts:[record(h,h.snap())]},prompts=[];
  h.db.rows.meta.set('bookingHistory',history);target.c.confirm=message=>{prompts.push(message);return prompts.length===1?restore:true;};
  await target.importFile({size:100,text:async()=>JSON.stringify(dump)});
  assert(prompts[0].includes('geen duurzame boekingshistorie'),'oude back-up zonder waarschuwing');
...
let failures=0;for(const [name,run] of checks)try{await run();console.log('PASS '+name);}catch(e){failures++;console.log('FAIL '+name+': '+e.message);}console.log(`${checks.length-failures}/${checks.length} Phase W checks passed`);process.exitCode=failures?1:0;
